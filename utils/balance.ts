...
import { SISHI, SISHI_BNB_FARM, SISHI_BNB_TOKEN, SISHI_TOKEN, MASTERCHEF_CONTRACT, WBNB_TOKEN } from "./constants";
...
export const getTotalStaked = async (address: string, block: string): Promise<number> => {
  ...
  try {
    ...
    const cakeContract = getContract(bep20ABI, SISHI, true);
  ...
  try {
    ...
    const cakeBnbContract = getContract(pairABI, SISHI_BNB_FARM, true);
    ...
    const pair: Pair = new Pair(
      new TokenAmount(SISHI_TOKEN, reservesLP._reserve0.toString()),
      new TokenAmount(WBNB_TOKEN, reservesLP._reserve1.toString())
    );
    const cakeLPBalance = pair.getLiquidityValue(
      pair.token0,
      new TokenAmount(SISHI_BNB_TOKEN, totalSupplyLP.toString()),
      new TokenAmount(SISHI_BNB_TOKEN, cakeBnbBalance.amount.toString()),
      false
    );
